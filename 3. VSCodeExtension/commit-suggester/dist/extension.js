...
const cp = __importStar(__webpack_require__(80));
...
async function startLlamaServer() {
    ...
    const serverPath = config.get('llamaServerPath');
    const modelPath = config.get('modelPath');
    const port = config.get('port') ?? 8081;
    const gpuLayers = config.get('gpuLayers') ?? 0;
    if (!serverPath || !modelPath) {
        vscode.window.showErrorMessage('Commit Suggester: Podesi putanje u Settings (commitSuggester.llamaServerPath i commitSuggester.modelPath)');
        return false;
    }
...
function getStagedDiff(repoPath) {
    try {
        const stat = (0, child_process_1.execSync)('git diff --staged --stat', {
            cwd: repoPath, encoding: 'utf-8', maxBuffer: 1024 * 1024
        }).trim();
        const patch = (0, child_process_1.execSync)('git diff --staged --patch', {
            cwd: repoPath, encoding: 'utf-8', maxBuffer: 1024 * 1024
        }).trim();
...
function isInitialCommit(repoPath) {
    try {
        (0, child_process_1.execSync)('git rev-parse HEAD', { cwd: repoPath, encoding: 'utf-8' });
        return false;
    }
    catch {
        return true;
...
async function suggestCommitMessage(diff, isInitial, projectName) {
...
${diff.slice(0, 6000)}`;
    const config = vscode.workspace.getConfiguration('commitSuggester');
    const LLAMA_URL = `http://localhost:${config.get('port') ?? 8081}/v1/chat/completions`;
    const response = await axios_1.default.post(LLAMA_URL, {
        model: 'local',
    ...
    const disposable = vscode.commands.registerCommand('commit-suggester.suggest', async () => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const action = await vscode.window.showInformationMessage(`💡 Predlog: "${suggestion}"`, 'Kopiraj', 'Upiši u SCM', 'Odbaci');
                if (action === 'Kopiraj') {
...
var FormData$1 = __webpack_require__(3);
...
var http2 = __webpack_require__(64);
var util = __webpack_require__(5);
var followRedirects = __webpack_require__(65);
